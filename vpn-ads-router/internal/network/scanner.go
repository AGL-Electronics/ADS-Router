...
var scannerlogger = logger.GetLogger()
...
func PlcDiscover() string { //check common beckhoff ports to identify the plc based on open ports, this to filter out false positives, if false pos still occur add more ports to fingerprint.
	if BindPlcAddr != "" {
		if ValidateBind(BindPlcAddr) {
			scannerlogger.Info(logger.ComponentNetwork, "PLC DISC: Using cached PLC Address:", BindPlcAddr)
			return BindPlcAddr
		}
	}
	...
	for i := 1; i <= 254; i++ {
		BaseIp := fmt.Sprintf("%s%d", scannerSubnet, i)
		matched := true
